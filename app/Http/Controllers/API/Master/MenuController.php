...
class MenuController extends Controller
{
    ...
    public function store(Request $req)
    {
        $validator = Validator::make($req->all(), [
            ...
            'isSubMenu' => 'required|numeric'
        ]);
        if ($validator->fails())
            return responseMsgs(false, $validator->errors(), []);
        try {
            $isExists = $this->_mMenus->readMenuGroup($req);
            if (collect($isExists)->isNotEmpty())
                throw new Exception("Menu Name Already existing");

            $isExists1 = $this->_mMenus->readMenuOrderGroup($req);
            if (collect($isExists1)->isNotEmpty())
                throw new Exception("Menu Order No Already existing");

            $metaReqs = [
                'menu_name' => Str::title($req->menuName),
                'order_no' => $req->orderNo,
                'is_sub_menu' => $req->isSubMenu,
            ...
            $this->_mMenus->store($metaReqs);
            $data = ['Menu Name' => $req->menuName, 'Order No' => $req->orderNo];
            $queryTime = collect(DB::getQueryLog())->sum("time");
            return responseMsgsT(true, "Records Added Successfully", $data, "M_API_33.1", $queryTime, responseTime(), "POST", $req->deviceId ?? "");
        } catch (Exception $e) {
            return responseMsgs(false, $e->getMessage(), [], "", "M_API_33.1", responseTime(), "POST", $req->deviceId ?? "");
        }
    }
    ...
    public function edit(Request $req)
    {
        ...
        try {
            ...
            if ($isExists && $isExists->where('id', '!=', $req->id)->isNotEmpty())
                throw new Exception("Menu Already existing");
            $getData = $this->_mMenus::findOrFail($req->id);
            $metaReqs = [
                'menu_name' => Str::title($req->menuName),
                'order_no' => $req->orderNo,
                'is_sub_menu' => $req->isSubMenu,
            ...
            $data = ['Menu Name' => $req->menuName, 'Order No' => $req->orderNo];
            $queryTime = collect(DB::getQueryLog())->sum("time");
            return responseMsgsT(true, "Records Updated Successfully", $data, "M_API_33.2", $queryTime, responseTime(), "POST", $req->deviceId ?? "");
        } catch (Exception $e) {
    ...
    public function show(Request $req)
    {
        ...
        if ($validator->fails())
            return responseMsgs(false, $validator->errors(), []);
        try {
            $show = $this->_mMenus->getGroupById($req->id);
    ...
    public function retrieveAll(Request $req)
    {
        try {
            ...
            $queryTime = collect(DB::getQueryLog())->sum("time");
            return responseMsgsT(true, "View All Records", $list, "M_API_33.4", $queryTime, responseTime(), "POST", $req->deviceId ?? "");
        } catch (Exception $e) {
    ...
    public function delete(Request $req)
    {
        ...
        try {
            ...
            $delete = $this->_mMenus::findOrFail($req->id);
    ...
    public function search(Request $req)
    {
        ...
        if ($validator->fails())
            return responseMsgs(false, $validator->errors(), []);
        try {
            $getData = $this->_mMenus->searchByName($req);
            $perPage = $req->perPage ? $req->perPage : 10;
            $paginater = $getData->paginate($perPage);
            // if ($paginater == "")
            //     throw new Exception("Data Not Found");
