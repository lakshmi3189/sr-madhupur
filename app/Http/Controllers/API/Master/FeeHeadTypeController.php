 ...
 * | Description- Fee Head Type CRUDS Operations
 * | Code Status- Closed
 */
...
class FeeHeadTypeController extends Controller
{
    ...
    public function store(Request $req)
    {
        ...
        try {
            $isGroupExists = $this->_mFeeHeadTypes->readFeeHeadTypeGroup($req);
            if (collect($isGroupExists)->isNotEmpty())
                throw new Exception("Fee Head Type Already Existing");
            $metaReqs = [
                'fee_head_type' => Str::title($req->feeHeadType),
                'school_id' => '0',
            ...
            $data = ['Fee Head Type Name' => $req->feeHeadType];
            $queryTime = collect(DB::getQueryLog())->sum("time");
            return responseMsgsT(true, "Records Added Successfully", $data, "M_API_2.1", $queryTime, responseTime(), "POST", $req->deviceId ?? "");
        } catch (Exception $e) {
    ...
    public function edit(Request $req)
    {
        ...
        if ($validator->fails())
            return responseMsgs(false, $validator->errors(), []);
        try {
            $isExists = $this->_mFeeHeadTypes->readFeeHeadTypeGroup($req);
            ...
            $data = ['Fee Head Type Name' => $req->feeHeadType];
            $queryTime = collect(DB::getQueryLog())->sum("time");
            return responseMsgsT(true, "Records Updated Successfully", $data, "M_API_2.2", $queryTime, responseTime(), "POST", $req->deviceId ?? "");
        } catch (Exception $e) {
    ...
    public function show(Request $req)
    {
        ...
        if ($validator->fails())
            return responseMsgs(false, $validator->errors(), []);
        try {
            $show = $this->_mFeeHeadTypes->getGroupById($req->id);
    ...
    public function delete(Request $req)
    {
        ...
        try {
            ...
            $delete = $this->_mFeeHeadTypes::findOrFail($req->id);
    ...
    public function search(Request $req)
    {
        ...
        if ($validator->fails())
            return responseMsgs(false, $validator->errors(), []);
        try {
            $getData = $this->_mFeeHeadTypes->searchByName($req);
            $perPage = $req->perPage ? $req->perPage : 10;
            $paginater = $getData->paginate($perPage);
            // if ($paginater == "")
            //     throw new Exception("Data Not Found");
            ...
            $queryTime = collect(DB::getQueryLog())->sum("time");
            return responseMsgsT(true, "View Searched Records", $list, "M_API_2.7", $queryTime, responseTime(), "POST", $req->deviceId ?? "");
        } catch (Exception $e) {
            return responseMsgs(false, $e->getMessage(), [], "", "M_API_2.7", responseTime(), "POST", $req->deviceId ?? "");
        }
    }
}
